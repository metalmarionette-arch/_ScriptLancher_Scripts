...
(function AnimPropSelect(thisObj) {
    ...
    function buildUI(thisObj) {
        ...
        var cbSel = r1.add("checkbox", undefined, "プロパティを選択");
        cbSel.value = state.doSelectProps;
        setTip(cbSel, "実行時に対象プロパティを選択状態にします（表示と併用可）。");
        ...
        var r3 = opt.add("group");
        r3.orientation = "row";
        r3.alignChildren = ["left", "center"];

        var cbKeyCursor = r3.add("checkbox", undefined, "カーソルのフレームにあるキーを選択");
        cbKeyCursor.value = state.selectKeysAtCursor;
        setTip(cbKeyCursor, "ONのとき、現在時間（カーソル）のフレーム±Nfrのキーフレームを選択します。");

        var stRange = r3.add("statictext", undefined, " ±");
        setTip(stRange, "キー選択の範囲指定（±フレーム）。");

        var etRange = r3.add("edittext", undefined, String(state.cursorKeyRangeFrames));
        etRange.characters = 4;
        setTip(etRange, "キー選択範囲（±フレーム数）。例：2 → 現在フレーム±2fr");

        var stRange2 = r3.add("statictext", undefined, "fr");
        setTip(stRange2, "フレーム数");

        function updateRangeEnabled() {
            etRange.enabled = cbKeyCursor.value;
            stRange.enabled = cbKeyCursor.value;
            stRange2.enabled = cbKeyCursor.value;
        }
        function updateClearEnabled() {
            cbClear.enabled = cbSel.value;
        }
        updateRangeEnabled();
        updateClearEnabled();
        ...
        cbSel.onClick = function () {
            state.doSelectProps = cbSel.value;
            updateClearEnabled();
        };

        cbClear.onClick = function () { state.clearBeforeSelect = cbClear.value; };

        cbKeyCursor.onClick = function () {
            state.selectKeysAtCursor = cbKeyCursor.value;
            updateRangeEnabled();
        };
